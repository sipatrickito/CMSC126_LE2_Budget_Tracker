...
def home(request):
    ...
    total_income = (
        entries.filter(type="income").aggregate(Sum("amount"))["amount__sum"] or 0
    )
    total_expenses = (
        entries.filter(type="expense").aggregate(Sum("amount"))["amount__sum"] or 0
    )
    ...
    for expense in expenses_by_category:
        category_expenses.append(
            {"name": expense["category__name"], "total": float(expense["total"])}
        )
        category_totals.append(
            {"category": expense["category"], "total_spent": expense["total"]}
        )
